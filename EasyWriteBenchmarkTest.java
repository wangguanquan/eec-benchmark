...
public class EasyWriteBenchmarkTest {
    ...
    private static void easyWrite(int n, Path subPath) {
        ...
        ExcelWriter excelWriter = EasyExcel.write(subPath.toFile()).withTemplate(path.getParent().resolve("fill.xlsx").toFile()).build();
        WriteSheet writeSheet = EasyExcel.writerSheet().build();
        for (int j = 0, p = 1000, c = n / p; j < c; j++) {
            excelWriter.fill(RandomDataProvider.randomData(p), writeSheet);
        }
        excelWriter.finish();
        System.out.println(subPath.getFileName() + " [Easyexcel] write finished. 耗时(ms): " + (System.currentTimeMillis() - start));
    }
}
